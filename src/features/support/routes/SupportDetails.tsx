...
const SupportDetails = () => {
  ...
  if (error) return <div className="p-4 text-sm text-red-600">Failed to load data</div>;
 ...
 const handleUpdateTicket = async () => {
  ...
  const handleUpdateFieldService = async () => {
  try {
    const response = await updateTicketServices(tenantId, updatedFieldPayload);
    if (response) {
      toast.success('Field service updated successfully');
      setOpenSupportVisit(false);
    }
  } catch (error) {
    toast.error('Failed to update field service');
  }
};
  ...
  return (
    <FormProvider {...methods}>
      {(openSupportIssue || openSupportVisit) && (
        <div className="px-4 py-2">
          <ActionHeader
            handleSubmit={openSupportIssue ? handleUpdateTicket : handleUpdateFieldService}
            handleCancel={handleCancel}
          />
        </div>
      )}
